  ...
  */
  ...
  let arcadeMusicMuted=false;
  function getArcadeAudio(){return document.getElementById("arcadeMusic");}
  function updateArcadeMusicBtn(){
    const btn=document.getElementById("arcadeMusicBtn");
    const icon=document.getElementById("arcadeMusicIcon");
    const label=document.getElementById("arcadeMusicLabel");
    if(!btn) return;
    btn.classList.toggle("is-muted", arcadeMusicMuted);
    if(icon) icon.textContent=arcadeMusicMuted?"🔇":"🔊";
    if(label) label.textContent=arcadeMusicMuted?"Muted":"Music";
    btn.setAttribute("aria-label", arcadeMusicMuted?"Unmute music":"Mute music");
  }
  function playArcadeMusic(){
    const audio=getArcadeAudio();
    if(!audio) return;
    audio.muted=arcadeMusicMuted;
    if(arcadeMusicMuted){audio.pause();updateArcadeMusicBtn();return;}
    const p=audio.play();
    if(p&&typeof p.catch==="function") p.catch(()=>{/* autoplay may be blocked until user gesture */});
    updateArcadeMusicBtn();
  }
  ...
  function toggleArcadeMusic(){
    ...
    if(audio){
      audio.muted=arcadeMusicMuted;
      if(arcadeMusicMuted) audio.pause();
      else{
  ...
  function navigate(view){
    document.querySelectorAll(".page").forEach(page=>page.classList.toggle("active",page.id===view+"View"));
    document.querySelectorAll("[data-action='navigate']").forEach(button=>button.classList.toggle("active",button.dataset.view===view));
    if(view==="game") playArcadeMusic();
  ...
  if(typeof document!=="undefined"){
    document.addEventListener("click",event=>{
      ...
      else if(action==="toggle-arcade-music")toggleArcadeMusic();
